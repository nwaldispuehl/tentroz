...
public class TripleChunk extends DraggablePiece {
  ...
  static {
    ...
    fields.add(new Field(FIELD_SIZE, GRID_SIZE, 2 * GRID_SIZE));

    fields.add(new Field(FIELD_SIZE, 2 * GRID_SIZE, 0));
    fields.add(new Field(FIELD_SIZE, 2 * GRID_SIZE, GRID_SIZE));
    fields.add(new Field(FIELD_SIZE, 2 * GRID_SIZE, 2 * GRID_SIZE));
  }

  public TripleChunk() {
    super(TRIPLE_CHUNK, fields);
  }
}
